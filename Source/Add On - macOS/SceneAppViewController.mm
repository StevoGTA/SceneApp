...
@property (nonatomic, assign)	CSceneAppPlayer*	sceneAppPlayer;
...
+ (TArray<CString>) scenePackageFilenames
{
	...
	for (NSURL* URL in
		...
		if ([filename hasPrefix:@"Scenes_macOS_"])
			// Found scene package
						...
						sceneAppPlayerCreationProc
{
	...
	if (self != nil) {
		...
		((NSView<SceneAppGPUView>*) self.view).mouseDownProc = ^(NSEvent* event){
			...
			weakSelf.sceneAppPlayer->mouseDown(
		...
		((NSView<SceneAppGPUView>*) self.view).mouseDraggedProc = ^(NSEvent* event){
			...
			// Inform SceneAppPlayer
			weakSelf.sceneAppPlayer->mouseDragged(
		...
		((NSView<SceneAppGPUView>*) self.view).mouseUpProc = ^(NSEvent* event){
			...
			// Inform SceneAppPlayer
			weakSelf.sceneAppPlayer->mouseUp(SSceneAppPlayerMouseUpInfo(S2DPoint32(point.x, point.y)));
		};

		((NSView<SceneAppGPUView>*) self.view).periodicProc = ^(UniversalTime outputTime){
			// Inform SceneAppPlayer
			weakSelf.sceneAppPlayer->handlePeriodic(outputTime);
		};
...
{
	self.sceneAppPlayer->loadScenes(scenePackageInfo);
}
...
{
	// Get info
	NSURL*	nsURL = [NSURL URLWithString:(NSString*) CFBridgingRelease(eStringCopyCFStringRef(url.getString()))];
